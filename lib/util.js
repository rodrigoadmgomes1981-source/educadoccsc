...
/** Percentual do vídeo que precisa ser assistido para liberar o certificado. */
export const COMPLETION=0.9;

/** Leitura mínima do PDF quando a aula também tem vídeo (o vídeo já cobre a carga horária). */
export const PDF_MIN_SECONDS=120;

/** Tempo mínimo de leitura do PDF, em segundos. */
export function pdfRequiredSeconds(lesson){
  const hasVideo=!!(lesson?.video_provider&&lesson?.video_id);
  if(hasVideo)return PDF_MIN_SECONDS;
  const workload=Number(lesson?.workload_minutes||0)*60;
  return workload>0?Math.round(workload*COMPLETION):PDF_MIN_SECONDS;
}

/**
 * Regra única de conclusão da aula, usada pelo servidor e espelhada na tela.
 * Vídeo: 90% do tempo de reprodução. PDF: tempo mínimo de leitura + declaração.
 * Tendo os dois, a aula só conclui quando as duas exigências forem cumpridas.
 */
export function evaluateLesson(lesson,progress){
  const hasVideo=!!(lesson?.video_provider&&lesson?.video_id);
  ...
  const pdfRequired=pdfRequiredSeconds(lesson);

  const videoOk=!hasVideo||(duration>0&&watched>=duration*COMPLETION);
  const pdfTimeOk=!hasPdf||pdfSeconds>=pdfRequired;
  const pdfOk=!hasPdf||!!progress?.pdf_confirmed_at;

  const videoPercent=hasVideo&&duration>0?Math.min(100,Math.round(watched/duration*100)):0;
  const pdfPercent=hasPdf?(pdfOk?100:Math.min(100,Math.round(pdfSeconds/pdfRequired*100))):0;
  const parts=[];
  if(hasVideo)parts.push(videoPercent);
  if(hasPdf)parts.push(pdfPercent);
  const percent=parts.length?Math.round(parts.reduce((a,b)=>a+b,0)/parts.length):0;

  return {
    hasVideo,hasPdf,videoOk,pdfOk,pdfTimeOk,
    pdfRequired,pdfSeconds,videoPercent,pdfPercent,percent,
    complete:(hasVideo||hasPdf)&&videoOk&&pdfOk
  };
}
...
export function missingMessage(state){
  const falta=[];
  if(!state.videoOk)falta.push(`assistir ${Math.round(COMPLETION*100)}% do vídeo (assistido: ${state.videoPercent}%)`);
  if(state.hasPdf&&!state.pdfOk){
  ...
  if(!state.hasVideo&&!state.hasPdf)return 'Esta aula não tem vídeo nem material em PDF, então não pode ser concluída pelo profissional.';
